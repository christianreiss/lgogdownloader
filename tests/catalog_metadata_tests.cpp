...
int main()
{
    ...
    Json::Value partial_dlc(Json::objectValue);
    partial_dlc["dlcs"] = Json::Value(Json::objectValue);
    partial_dlc["dlcs"]["products"] = Json::Value(Json::arrayValue);
    for (unsigned int i = 0; i < 3; ++i)
        partial_dlc["dlcs"]["products"].append(Json::Value(Json::objectValue));
    partial_dlc["expanded_dlcs"] = Json::Value(Json::arrayValue);
    partial_dlc["expanded_dlcs"].append(Json::Value(Json::objectValue));
    expect(!CatalogMetadata::hasCompleteDlcExpansion(partial_dlc),
           "expansion shorter than the listed DLCs is incomplete metadata");
    for (unsigned int i = 0; i < 2; ++i)
        partial_dlc["expanded_dlcs"].append(Json::Value(Json::objectValue));
    expect(CatalogMetadata::hasCompleteDlcExpansion(partial_dlc),
           "expansion covering every listed DLC is complete metadata");
