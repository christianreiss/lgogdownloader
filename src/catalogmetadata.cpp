...
bool CatalogMetadata::hasCompleteDlcExpansion(const Json::Value& productJson)
{
    if (!productJson.isObject())
        return false;
    if (!productJson["dlcs"].isObject())
        return true;

    const Json::Value& expanded = productJson["expanded_dlcs"];
    if (!expanded.isArray())
        return false;

    // galaxyAPI::getProductInfo expands more than 45 DLCs in several requests and
    // appends whatever each one returned, so a partially expanded list is an array
    // that is merely short rather than absent. Compare it against the list it
    // expands, otherwise DLC files missing from a half-resolved catalog would look
    // like files GOG no longer offers.
    const Json::Value& listed = productJson["dlcs"]["products"];
    if (!listed.isArray())
        return true;

    return expanded.size() >= listed.size();
}
